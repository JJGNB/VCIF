...
class BackboneModel(SRModel):
    def init_training_settings(self):
        self.net_g.train()
        train_opt = self.opt['train']
        self.source_type=self.opt['network_g']['source']
        ...
        if train_opt.get('tv_opt'):
            ...
            self.cri_tv=None
        if self.cri_pix is None and self.cri_perceptual is None and self.cri_tv is None:
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.source_type=="ir":
            lq_en=TF.adjust_contrast(self.lq,1.5)
            self.gt=lq_en
